...
train_spec(){
  ...
  python3 -m spec train \
      --seed 42 \
      --gpu-id 3  \
      --output-dir "runs/test-imdb-unfreezed-embs-${activation}/" \
      --save "data/saved-models/test-imdb-unfreezed-embs-${activation}/" \
...
predict_spec(){
  ...
  python3 -m spec predict \
      ...
      --load "data/saved-models/test-imdb-unfreezed-embs-${activation}/" \
      --corpus imdb \
      --test-path "data/corpus/imdb/test/" \
      --output-dir "data/predictions/test-imdb-unfreezed-embs-${activation}/" \
      --dev-batch-size 4
}
