...
communicate_spec(){
  ...
  python3 -m spec communicate_translation \
                          --seed 42 \
                          --gpu-id 0  \
                          --output-dir "runs/communicate-grad-iwslt-${activation}-new/" \
                          --save "data/saved-models/communicate-grad-iwslt-${activation}-new/" \
                          --save-explanations "data/explanations/iwslt_grad_test_${activation}_new_${topk}.txt" \
                          --max-explanations 2000 \
                          --print-parameters-per-layer \
                          --final-report \
                          \
                          --corpus iwslt \
                          --train-path "data/saved-translation-models/iwslt-ende-bahdanau-${activation}-new/grad-train/train" \
                          --dev-path "data/saved-translation-models/iwslt-ende-bahdanau-${activation}-new/grad-dev/dev" \
                          --test-path "data/saved-translation-models/iwslt-ende-bahdanau-${activation}-new/grad-test/test" \
                          --max-length 100 \
                          --min-length 0 \
                          \
                          --vocab-size 9999999 \
