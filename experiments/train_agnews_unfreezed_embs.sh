...
train_spec(){
  ...
  python3 -m spec train \
      --seed 42 \
      --gpu-id 3  \
      --output-dir "runs/test-agnews-unfreezed-embs-${activation}/" \
      --save "data/saved-models/test-agnews-unfreezed-embs-${activation}/" \
...
predict_spec(){
  ...
  python3 -m spec predict \
      ...
      --load "data/saved-models/test-agnews-unfreezed-embs-${activation}/" \
      --corpus agnews \
      --test-path "data/corpus/agnews/test.xml" \
      --output-dir "data/predictions/test-agnews-unfreezed-embs-${activation}/" \
      --dev-batch-size 4
}
