...
train_spec(){
  ...
  python3 -m spec train \
      --seed 42 \
      --gpu-id 3  \
      --output-dir "runs/test-sst-unfreezed-embs-${activation}/" \
      --save "data/saved-models/test-sst-unfreezed-embs-${activation}/" \
...
predict_spec(){
  ...
  python3 -m spec predict \
      ...
      --load "data/saved-models/test-sst-unfreezed-embs-${activation}/" \
      --corpus sst \
      --test-path "data/corpus/sst/test.txt" \
      --output-dir "data/predictions/test-sst-unfreezed-embs-${activation}/" \
      --dev-batch-size 4
}
